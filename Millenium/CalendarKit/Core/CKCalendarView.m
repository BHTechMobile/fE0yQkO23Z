...
- (void)setFrame:(CGRect)frame animated:(BOOL)animated
{
    ...
    if (animated) {
        ...
        //        NSLog(@"frame display : with : %f, height : %f",frame.size.width,frame.size.height);
        //
        if ([[self delegate] respondsToSelector:@selector(calendarView:frameCalendarView:)]) {
            [[self delegate] calendarView:self frameCalendarView:frame];
        }
    }
    else
    {
    ...
    for (NSUInteger row = 0; row < rowCount; row++) {
        for (NSUInteger column = 0; column < columnCount; column++) {
            ...
            if (cellIndex == [self selectedIndex]) {
                ...
                if (_checkDateSelected) {
                    dateSelected = [dateFormatter stringFromDate:_dateSelected];
                }else
                {
                    dateSelected = [dateFormatter stringFromDate:workingDate];
                }
                
                if ([dateSelected compare:dateNow]  == NSOrderedDescending || [dateSelected compare:dateNow]  == NSOrderedSame) {
                [cell setSelected];
                } else
                {
                    [cell setDeselected];
...
- (void)setDate:(NSDate *)date animated:(BOOL)animated
{
    ...
    if (minimumIsBeforeMaximum) {
        ...
        if ([[self delegate] respondsToSelector:@selector(calendarView:willSelectDate:withAnimated:)]) {
            [[self delegate] calendarView:self willSelectDate:date withAnimated:animated];
        }
        _previousDate = _date;
        _date = date;
        
        if ([[self delegate] respondsToSelector:@selector(calendarView:didSelectDate:withAnimated:)]) {
            [[self delegate] calendarView:self didSelectDate:date withAnimated:animated];
        }
...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event
{
    ...
    if(CGRectContainsPoint([self _rectForCellsForDisplayMode:_displayMode], point)){
        ...
        _dateSelected = [self _dateFromIndex:index];
        if ([self _dateIsAfterMaximumDate:_dateSelected]) {
            index = [self _indexFromDate:[self maximumDate]];
        }
        else if([self _dateIsBeforeMinimumDate:_dateSelected])
        {
        ...
        [self setSelectedIndex:index];
            ...
            for (CKCalendarCell *cell in [self usedCells]) {
                if ([cell index] == [self selectedIndex]) {
                    ...
                    [dateFormatter setDateFormat:@"yyyy-MM-dd"];
                    NSString *dateNow = [dateFormatter stringFromDate:[NSDate date]];
                    NSString *dateSelected = [dateFormatter stringFromDate:_dateSelected];
                    if ([dateSelected compare:dateNow]  == NSOrderedDescending || [dateSelected compare:dateNow]  == NSOrderedSame) {
